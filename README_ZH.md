...
| 问题 | 解决方案 |
|------|---------|
| `uv run abaqus-control-mcp-server` 无输出 | 这是正常的，stdio MCP Server 不输出日志 |
| 按 Enter 时报 `JSON parse error` | 不要向 stdio 服务器输入空行 |
| `Module abaqusGui can only be used in Abaqus/CAE GUI` | 通过 Plug-ins 菜单启动插件，不用 File -> Run Script |
| 连接 `timed out` 超时 | 检查插件日志 `$env:TEMP\abaqus_mcp_gui_plugin.log` |
| 模型未出现在 GUI 中 | 验证 `abaqus-control-check` 返回 `MainThread` 和模型列表 |
...
有问题？有建议？[提出 Issue](https://github.com/yourusername/abaqus-control-mcp/issues) 或在 GitHub Discussions 中讨论。
