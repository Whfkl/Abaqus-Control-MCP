...
使用 `pip` 或 `uv` 都可以。安装后会得到 MCP 服务、连接检查、插件安装器和诊断命令。
...
#### 2. 安装 Abaqus/CAE GUI 插件

发布后的包内已经包含插件安装器，直接运行：
...
#### 3. 重启 Abaqus/CAE 并验证连接

重启 Abaqus/CAE，然后通过菜单启动插件：
...
| 问题 | 解决方案 |
|------|---------|
| 找不到 `abaqus-control-install-plugin` | 在同一个 Python 环境中重新安装该包，然后运行 `abaqus-control-doctor` 确认脚本是否在 PATH 中 |
| `command not found: abaqus-control-mcp-server` | 包可能安装到了别的 Python 环境。运行 `abaqus-control-doctor`，或使用启动客户端时同一个解释器重新安装 |
| 插件安装失败或不清楚是否成功 | 运行 `abaqus-control-install-plugin --target-dir <path>` 查看 JSON 结果；如果需要，PowerShell 脚本可以加 `-Force` 覆盖 |
